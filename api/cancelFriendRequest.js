...
*/
router.post("/cancelFriendRequest", bodyParser.json(), async (req, res) => {
    ...
    {
        let [exists, e] = await friendRequests.existsExactRequest(id, target)
        if (e) {
    ...
    let e = await friendRequests.declineRequest(id, target)
    if (e) {
