...
pending (list of numbers): a list of user ids that have sent you requests (currently pending)
...
1013 - Invalid cookie
*/
router.get("/getPendingFriendRequests", bodyParser.urlencoded({extended:true}), async (req, res) => {
    let id = req.query["id"]
    ...
    let [pending, e] = await friendRequests.getPendingRequests(id)
    if (e || !pending) {
        res.json({success:false, errorCode:1014, errorMessage:"An error occurred when trying to get pending requests"})
        return
    }
